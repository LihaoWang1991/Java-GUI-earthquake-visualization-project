...
public class EarthquakeCityMap extends PApplet {
	...
	private void addKey() {	
		...
		fill(color(255, 0, 0));
		triangle(50, 125-5, 50-4, 125+3, 50+4, 125+3);
		fill(0, 0, 0);
		...
		fill(0, 0, 0);
		text("Land Quaker", 75, 145);
		
		fill(color(255, 255, 255));
		rect(50-5, 165-5, 10, 10);
		fill(0, 0, 0);
		text("Ocean Quaker", 75, 165);
	...
	private void printQuakes() 
	{
		...
		// added by Lihao WANG: count earthquake numbers for each country
		for (Marker cm : countryMarkers)
		{
			int countryQuakeCounter= 0;
			for (Marker em : quakeMarkers)
			{
				if ((String)cm.getProperty("name")==(String)em.getProperty("country"))
				{
					countryQuakeCounter=countryQuakeCounter+1;
				}
			}	
			
			if (countryQuakeCounter > 0) {
			System.out.println((String)cm.getProperty("name")+": "+countryQuakeCounter);
			}
			
		}
		// added by Lihao WANG: count earthquake numbers for ocean
